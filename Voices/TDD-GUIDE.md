...
```swift
func startRecording() {
    logError("TRACE: startRecording — expected audio capture to begin, but no data flowing")
...
func startRecording() {
    recorder.start()
    log("TRACE: startRecording — audio capture active, buffer size = \(recorder.bufferSize)")
    log("Recording started")
}
```
...
`ContentView` holds recording/listening stubs, `@State` bools, and `onChange` dispatch. `RecordButton` and `ListenButton` use `@Binding` — the view mutates state directly. All behavior lives in the view layer.
...
│  - Data model: chunk state machine               │
...
Migration path:
1. Extract `VoicesViewModel` (`@Observable @MainActor`), move recording/listening logic out of ContentView.
2. Make buttons dumb (data + callback instead of `@Binding`).
3. Add data model as a separate `@Observable` store, injected into the view model.
...
Data in, callbacks out. No `@State` for business logic, no `Task` creation.

```swift
// Target: RecordButton takes data + callback, not @Binding
struct RecordButton: View {
    let isRecording: Bool
    let onTap: () -> Void
    // body renders from isRecording, routes tap to onTap
}
```
...
```swift
func startRecording() {
    log("TRACE: startRecording — isRecording before = \(isRecording)")
    // ... existing logic ...
    log("TRACE: startRecording — isRecording after = \(isRecording)")
}
```
...
Example: `test_toggleRecording_whenNotRecording_startsRecording()`
